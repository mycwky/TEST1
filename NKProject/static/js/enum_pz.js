...
function reloadDT_DEVICE_DATA(data,w,DATAID_STATUS,xiaoshu) {
    ...
    $("#DT_Device").append("<div class='row' id='"+single_list_div+"'><div class='col-md-2' id='"+div_img+"'><div id='"+dt_img+"'></div></div><div style='padding-left:50px'class='col-md-10' id='"+ds_data+"'><div style='float: left' id='"+yc+"' ></div><div id='"+yx+"'></div></div>");
        ...
        if(response.obj.list.length>0){
            ...
            if(qy){
                for(var i =0;i<(divNum+1);i++){
                    ...
                    var table=$("<table class=' table-bordered' style='border: 5px solid slategrey; width:180px;margin: 0 10px'></table>")
                    for(var j=parseInt(maxRowNum)*i;j<parseInt(maxRowNum)*(i+1);j++){
                        if(j<list.length){
                            ...
                            let tr="<tr style='color: white;font-weight: lighter;font-size: 12px'><td>"+dDescr+"</td><td>"+parseFloat(list[j].v).toFixed(parseInt(xiaoshu))+"</td><td>["+dUnit+"]</td></tr>";
                            // console.log(tr)
                        }
                    }
                    divObj.append(table)
                ...
                for(var i =0;i<divNum;i++){
                    ...
                    var table=$("<table class=' table-bordered' style='border: 5px solid slategrey; width: 180px;margin-right:10px'></table>")
                    for(var j=parseInt(maxRowNum)*i;j<parseInt(maxRowNum)*(i+1);j++){
                        var dDescr;
                        if(list[j].dExt){
                            dDescr=list[j].dExt.dDescr
...
function reloadDT_DEVICE_DATA_1(w,d,xiaoshu,divId,type,titleName) {
    ...
    var div_img = "div_img"+random;
    var single_list_div = "single_list_div"+random;
    $("#"+divId).append("<div class='row'style='border:2px solid #91918d;width:93%' id='"+single_list_div+"'><div style='color:white;width: 100%;height: 30px;text-align: center'>"+titleName+"</div><div  id='"+ds_data+"'><div style='display: flex;' id='"+yc+"' ><div id='"+yx+"'></div></div>");
