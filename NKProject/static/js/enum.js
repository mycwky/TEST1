    ...
    var option = {
        ...
        myChart.on('click', function (params) {
            ...
            if(thisTag){
                ...
                getDT_DEVICE_DATA(w);//加载所选择的单体设备光字牌数据
            }
         });
}
...
function reloadDT_DEVICE_DATA(data,w,DATAID_STATUS,xiaoshu,) {
    ...
    $("#DT_Device").append("<div class='row' id='"+single_list_div+"'><div class='col-md-2' id='"+div_img+"'><div id='"+dt_img+"'></div></div><div style='padding-left:30px' class='col-md-10' id='"+ds_data+"'><div style='float: left;display: flex' id='"+yc+"' ></div><div id='"+yx+"'></div></div>");
    // $("#DT_Device").append("<div class='row' id='"+single_list_div+"' ><div class='col-md-12' id='"+ds_data+"'><div style='float: left;display: flex' id='"+yc+"' ></div><div id='"+yx+"'></div></div>");
    ...
    if( data.value.img != "undefined" && data.value.img!=""){
        ...
        if(DT_IMG){
            ...
            DT_IMG.style.width="150px"
            DT_IMG.style.marginTop="0px";
            DT_IMG.style.marginLeft="15px";
    ...
    $.ajax(settings_yc).done(function (response) {
        ...
        if(response.obj.list.length>0){
            ...
            var divObj = $("#DT_Device").children("#"+single_list_div).children("#"+ds_data).children("#"+yc);
            // var divObj=doc.getElementsByClassName("DT_DATA_DIV_YC");
            if(qy){
                for(var i =0;i<(divNum+1);i++){
                    //   var div_id=divObj[i].id;
                    var table=$("<table class=' table-bordered' style='border: 5px solid slategrey; width: 180px;margin-left: 10px'></table>")
                ...
                for(var i =0;i<divNum;i++){
                    ...
                    for(var j=parseInt(maxRowNum)*i;j<parseInt(maxRowNum)*(i+1);j++){
                        ...
                        table.append("<tr style='color: white;'><td>"+dDescr+"</td><td>"+parseFloat(list[j].v).toFixed(parseInt(xiaoshu))+"</td></tr>")
                    }
                    divObj.append(table)
                    // $("#"+div_id).append(table)
                }
...
function reloadDT_DEVICE_DATA_1(w,d,xiaoshu,divId,type,titleName) {

    $("#"+divId).empty();
    // $("#DT_DS_DATA_YX").empty();
    // $("#DT_DS_DATA_YC").empty();
    var yx = "DT_DS_DATA_YX"+divId;
    var yc = "DT_DS_DATA_YC"+divId;
    var ds_data = "DT_DS_DATA1"+divId;
    var single_list_div = "single_list_div"+divId;
    $("#"+divId).append("<div style='border:2px solid #91918d;width:93%' class='row' id='"+single_list_div+"'><div style='color:white;width: 100%;height: 30px;text-align: center'>"+titleName+"</div><div  id='"+ds_data+"'><div style='display: flex;' id='"+yc+"' ></div style='display: flex;'><div id='"+yx+"'></div></div>");
    ...
    spanObj.style.color="white";
    spanObj.setAttributeNode(spanObjID);
    //$("#"+divId).children("#"+single_list_div).children(" #"+div_img+" #"+dt_img).append(spanObj);
    //document.getElementById(img).appendChild(spanObj);
    //     $.ajax({
    //         "async": false,
    //         "crossDomain": true,
    ...
    //             "cache-control": "no-cache"
    //         },
    //         "data":"{w:'"+DId+"',d:'"+d+"',isRT:true,partyId:'208060df-d0fa-44bb-8fa3-d468b97d164a'}"
    //     }).done(function (response) {
    //         if(response.obj.list.length>0){
    //             if(response.obj.list[0].v<=0){
    //                 //spanObj.style.background="red";
    ...
    $.ajax(settings_yc).done(function (response) {
        ...
        // var doc=document.getElementById("DT_DS_DATA_YC").ch;
        var doc = $("#"+divId).children("#"+single_list_div).children("#"+ds_data).children("#"+yc);
        if(divNum<=0){
            var div=document.createElement('div');
            var divattr = document.createAttribute("class");
            var divId=document.createAttribute('id');
        ...
        if(response.obj.list.length>0){
            ...
            if(qy){
                for(var i =0;i<(divNum+1);i++){
                    ...
                    var table=$("<table class=' table-bordered' style='border: 5px solid slategrey;margin-left: 10px'></table>")
                    for(var j=parseInt(maxRowNum)*i;j<parseInt(maxRowNum)*(i+1);j++){
                        if(j<list.length){
                            var dDescr;var dUnit;
                            if(list[j].dExt && list[j].dExt!=null){
                                dDescr=list[j].dExt.dDescr
                                dUnit=list[j].dExt.dUnit;
                            }else{
                                dDescr="--"
                            ...
                            dUnit= dUnit==""?"--":dUnit;
                            if(list[j].v!=null && list[j].v!=""){
                                dUnit=dUnit==null?"--":dUnit
                                table.append("<tr style='color: white;font-weight: lighter;height: 46px'><td style='width: 180px'>"+dDescr+"</td><td>"+parseFloat(list[j].v).toFixed(parseInt(xiaoshu))+"</td><td>["+dUnit+"]</td></tr>")
                            }
                        }
                    }
                    divObj.append(table)
                ...
                for(var i =0;i<divNum;i++){
                    ...
                    for(var j=parseInt(maxRowNum)*i;j<parseInt(maxRowNum)*(i+1);j++){
                        ...
                        dUnit= dUnit==""?"--":dUnit;
                        table.append("<tr style='color: white;height: 46px'><td td style='width: 200px'>"+dDescr+"</td><td>"+parseFloat(list[j].v).toFixed(parseInt(xiaoshu))+"</td><td>["+dUnit+"]</td></tr>")
                    }
                    divObj.append(table)
                    // $("#"+div_id).append(table)
                }
    ...
    if(type==2){
        ...
        $.ajax(settings_yx).done(function (response) {
            ...
            var doc = $("#"+divId).children("#"+single_list_div).children("#"+ds_data).children("#"+yx);
            // var doc = $("#"+yx);
            if(divNum<=0){
                var div=document.createElement('div');
                var divattr = document.createAttribute("class");
            ...
            if(response.obj.list.length>0){
                ...
                // var divObj = $("#"+divId).children("#"+single_list_div).children("#"+ds_data).children("#"+yx);
                var divObj =$("#"+yc);
                if(qy){
                    for(var i =0;i<(divNum+1);i++){
                        // var div_id=divObj[i].id;
                        var table=$("<table class='table-bordered' style='border: 5px solid slategrey;text-align: center; width:145px;margin-left: 10px'></table>")
                        for(var j=parseInt(maxRowNum)*i;j<parseInt(maxRowNum)*(i+1);j++){
                            if(j<list.length){
                                if(parseInt(list[j].v)<=0){
                                    var dDescr;
                                    if(list[j].dExt && list[j].dExt!=undefined){
                                        dDescr=list[j].dExt.dDescr
                                    }else{
                                        dDescr="--"
                                    }
                                    table.append("<tr style='color: white;font-weight: lighter;font-size: 12px;height: 46px'><td>"+dDescr+"</td></tr>")
                                }else if(list[j].v==null){
                                    var dDescr;
                                    if(list[j].dExt && list[j].dExt!=undefined){
                                        dDescr=list[j].dExt.dDescr
                                    ...
                                    table.append("<tr style='color: white;font-weight: lighter;font-size: 12px;background-color: #c2c2c2;height: 46px'><td>"+dDescr+"</td></tr>")
                                }else{
                                    for(var e in D_IOT_EVENT_LEVEL){
                                        if(list[i].dExt){
                                            if(list[i].dExt.evL==D_IOT_EVENT_LEVEL[e].key){
                                                ...
                                                switch (D_IOT_EVENT_LEVEL[e].pkey) {
                                                    case "info":
                                                        ...
                                                        table.append("<tr style='background-color: green;color: white;font-weight: lighter;font-size: 12px;height: 46px'><td>"+dDescr+"</td></tr>")
                                                        break;
                                                    case "alert":
                                                        if(list[j].dExt && list[j].dExt!=null){
                                                            dDescr=list[j].dExt.dDescr
                    ...
                    for(var i =0;i<divNum;i++){
                        //var div_id=divObj[i].id;
                        var table=$("<table class='table-bordered' style='border: 5px solid slategrey;text-align: center;'></table>");
                        for(var j=parseInt(maxRowNum)*i;j<parseInt(maxRowNum)*(i+1);j++){
                            if(parseInt(list[j].v)<=0){
                                var dDescr;
                                if(list[j].dExt&& list[j].dExt!=null){
