...
$(function () {
  ...
  switch (whichHtml) {
    case "pz":
      dd=[{
        "text":"主页",
        "href":"NK_PZ_MAIN.html",
        "state":{
          "expanded":true
        },
      }];
      var htmlArr=["NK_page_pz.html","NK_page_pz.html","NK_page_pz.html","NK_page_pz.html","NK_page_pz.html","NK_page_pz.html"];
      var dd_2=[];
      for(var i in LINE){
        dd_2.push({
          "text":LINE[i].val,
...
function getData_Echarts(dataDateType,wd,showEchartsType,startDT,endDT,divType,divId,whetherToDisplayDataLabels,formatterValue,dataNameArray,deviceIdAndDataId,xiaoshu,dataType,showPercentYesOrNo) {
  let dataObj = "{partyId:'208060df-d0fa-44bb-8fa3-d468b97d164a'";
  var realTimeData=[];
  var interfanceNum;
  switch (dataDateType) {
    case "dataOfDay":
      interfanceNum=3;
      //dataObj += ",w:"+w+",d:"+d+",startDT:" + startDT+",endDT:" + endDT+",outSet:'NK001'}"
      dataObj += ",wd:"+wd+",startDT:" + startDT+",endDT:" + endDT+",outSet:'NK001'}"
      $.ajax({
        "async": true,
        "crossDomain": true,
        ...
        "data": "{partyId:'208060df-d0fa-44bb-8fa3-d468b97d164a',wd:"+wd+",isRT:true,outSet:'NK001'}"
      }).done(function (data) {
    ...
    case "dataOfMonth":
      switch (dataType) {
        case "echarts_fd":
          interfanceNum=25;
          dataObj += ",wd:"+wd+",startDT:" + startDT+",rptType:'m',outSet:'NK001'}"
          break;
        case "echarts_mr":
          interfanceNum=25;
          dataObj += ",wd:"+wd+",startDT:" + startDT+",rptType:'m',outSet:'NK001'}"
        ...
        case "echarts_alert":
          interfanceNum=5;
          dataObj += ",evL:"+w+",startDT:" + startDT+",endDT:"+endDT+",outSet:'NK001'}"
          break;
      }
      break;
    case "dataOfYear":
      switch (dataType) {
        case "echarts_fd":
          interfanceNum=25;
          dataObj += ",wd:"+wd+",startDT:" + startDT+",rptType:'y',outSet:'NK001'}"
          break;
        case "echarts_mr":
          interfanceNum=25;
          dataObj += ",wd:"+wd+",startDT:" + startDT+",rptType:'y',outSet:'NK001'}"
  ...
  if(wd!="[]"){
    $.ajax({
      ...
      var objArray=response.obj.list;
        ...
        //   let newdata=objArray[i].data;
        //   for(let j  in newdata){
        //     if(parseFloat(newdata[j].v)>70000){
        //       console.log()
        //       delete newdata[j]
        //     }
        //   }
              ...
              switch (dataDateType) {
                ...
                case "dataOfDay":
                  ...
                  for(var k=0;k<objArray[i].data.length;k++){
                    ...
                    if(parseFloat(objArray[i].data[k].t)<time && parseFloat(objArray[i].data[k].v)>100 ){
                        objArray[i].data[k].v="0";
                    }
                    var fz=70000;
                    if(divId=="echarts_div_2"){
                      fz=500
                    }
                    if(parseFloat(objArray[i].data[k].v)<fz){
                      list_1.push({
                        "value":[
          ...
          for(var o in list_1){
            ...
            for(var uu in deviceIdAndDataId){
              if(list_1[o].w==deviceIdAndDataId[uu].deviceId&&list_1[o].d==deviceIdAndDataId[uu].dataId){
                ...
                y.push({
                  name:deviceIdAndDataId[uu].dataName,
...
function getHistoryReport(startDT,endDT,objTag) {
  ...
  if(objTag=="main"){
    setting={
      ...
      "data": "{startDT:" + startDT + ",endDT:" + endDT + ",partyId:'208060df-d0fa-44bb-8fa3-d468b97d164a',evL:['alert1','alert2','alert3','fault1','fault2','fault3'],outSet:'NK001'}"
    }
  }else{
    setting={
      "async": true,
      ...
      "data": "{startDT:" + startDT + ",endDT:" + endDT + ",partyId:'208060df-d0fa-44bb-8fa3-d468b97d164a',evL:['alert1','alert2','alert3','fault1','fault2','fault3'],objTags:['"+objTag+"'],outSet:'NK001'}"
    }
  }
    ...
    if(response.resMsg=="信息获取成功"){
      if(response.obj.list.length>0){
        ...
        for(var i in valueArray){
          var tr=$("<tr><td>"+(parseInt(i)+1)+"</td><td>"+valueArray[i].hwName+"</td><td>"+valueArray[i].dDescr+"</td><td>"+valueArray[i].zty+"|"+valueArray[i].v+"</td><td>"+valueArray[i].t+"</td><td>"+valueArray[i].evL+"</td></tr>");
