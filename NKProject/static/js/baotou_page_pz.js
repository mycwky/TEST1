                    ...
                    falg:for(let i in databaseData){
                        if(databaseData[i].area == $("#areaId").val()){
                            ...
                            if(headType == divId){
                                ...
                                $("#xiaoShu").val(headData.headRatio);
                                break falg;
                            }else{
...
function saveHead(){
    $("#head-save-btn").click(function () {
        ...
        let xiaoshu=$('xiaoshu').val;
        if(headDeviceId && headDataId && headName && headUnit&&xiaoshu) {
            let json = {
...
function reloadAlertData(alertArray,startDT,endDT) {
    ...
    if(alertArray.length>2){
        $.ajax({
            ...
            //"data": "{evL:"+alertArray+",partyId:'208060df-d0fa-44bb-8fa3-d468b97d164a',startDT:1546272000000,endDT:1577808000000,outSet:'NK001'}"
            ...
            if(response.obj){
                if(response.obj.list.length>0){
                    ...
                    $("#alert_div table thead").append($("<tr style='background-color:rgb(238,238,238)'><th>设备描述</th><th>数据点描述</th><th>状态域描述</th><th>状态值描述</th><th>时间</th><th>事件等级</th></tr>"))
