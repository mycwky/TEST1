$(function(){
    //alert("进入了alert_gen.js")
    // window.parent.test()
    //业主字典接口
    let data2 = "{dicts:['D_IOT_OBJ_TAG','D_IOT_HW_MODEL'],partyId:'208060df-d0fa-44bb-8fa3-d468b97d164a'}";
    let respone2 = postRunAsyncToFJson(urlAlertTreeData, data2);
    for (var i in respone2.obj.list) {
        if (respone2.obj.list[i].name == "D_IOT_OBJ_TAG") {
            var tcarray = [];
            var yzcarray = [];
            D_IOT_OBJ_TAG = respone2.obj.list[i].data;
        ...
        } else if (respone2.obj.list[i].name == "D_IOT_HW_MODEL") {
            D_IOT_HW_MODEL = respone2.obj.list[i].data;
